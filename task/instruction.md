...
The unique published estimand, identification rules, schema, and rounding are in `/app/docs/CONTRACT.md`. Fit trees under `/app/fit/case_*/` ship the same input filenames as production plus a published `domain_cif.csv` worked example. They are unlabeled extracts, not a check-only oracle. The unique construction of risk sets, competing-event incidence, and the person linearized standard error is the one that reproduces every fit published table.
